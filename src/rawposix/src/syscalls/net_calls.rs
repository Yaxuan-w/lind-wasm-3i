...
pub fn select_syscall(
    ...
    errorfds_arg: u64,
    errorfds_cageid: u64,
    rposix_timeout_arg: u64,
    rposix_timeout_cageid: u64,
    arg6: u64,
    arg6_cageid: u64,
) -> i32 {
    let nfds = sc_convert_sysarg_to_i32(nfds_arg, nfds_cageid, cageid);
    let readfds_addr = sc_convert_uaddr_to_host(readfds_arg, readfds_cageid, cageid);
    let mut readfds = sc_convert_fdset(readfds_addr, readfds_cageid, cageid).unwrap();
    let writefds_addr = sc_convert_uaddr_to_host(writefds_arg, writefds_cageid, cageid);
    let mut writefds = sc_convert_fdset(writefds_addr, writefds_cageid, cageid).unwrap();
    let errorfds_addr = sc_convert_uaddr_to_host(errorfds_arg, errorfds_cageid, cageid);
    let mut errorfds = sc_convert_fdset(errorfds_addr, errorfds_cageid, cageid).unwrap();
    let timeout_addr = sc_convert_uaddr_to_host(rposix_timeout_arg, rposix_timeout_cageid, cageid);
    let rposix_timeout = sc_convert_duration_fromtimeval(timeout_addr,rposix_timeout_cageid, cageid).unwrap();

    if !(sc_unusedarg(arg6, arg6_cageid))
    {
        return syscall_error(Errno::EFAULT, "select_syscall", "Invalide Cage ID");
    }
...
pub fn getifaddrs_syscall(
    ...
    && sc_unusedarg(arg6, arg6_cageid))
    {
        return syscall_error(Errno::EFAULT, "getifaddrs_syscall", "Invalide Cage ID");
    }
