...
impl Instance {
    ...
    // -------------- AW --------------
    pub fn instantiate_with_grate(&self, mut store: impl AsContextMut<Data = T>, instantiate_type: InstantiateType) -> Result<Instance> {
        let mut store = store.as_context_mut();
        ...
        unsafe { Instance::new_started_impl_with_grate(&mut store, &self.module, imports.as_ref(), instantiate_type) }
    }

    pub(crate) unsafe fn new_started_impl_with_grate<T>(
        store: &mut StoreContextMut<'_, T>,
        module: &Module,
        imports: Imports<'_>,
        instantiate_type: InstantiateType,
        ...
        match instantiate_type {
            ...
            InstantiateType::InstantiateFirst(pid) => {
                ...
                // 1. set memory base address
                // 2. manually call mmap_syscall to set up the first memory region
                // TODO: tmp start grate id from 400
                let handle = store.0.instance(InstanceId::from_index(400));
                let defined_memory = handle.get_memory(wasmtime_environ::MemoryIndex::from_u32(0));
                let memory_base = defined_memory.base as usize;
